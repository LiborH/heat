...
public class SensorsFacade {
  ...
  public Response registerSensor(@PathParam("id") String id) {
    SensorService.getInstance().registerSensor(id);
    return Response.status(201).build();
  }
  
  /**
   * Receives a reading of the sensor node. Note: Since sensor nodes do not
   * track time on their own, this method adds a timestamp to the reading.
